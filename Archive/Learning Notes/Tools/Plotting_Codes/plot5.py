...
for env_id5 in sorted(data5.keys()):
    print('exporting {}'.format(env_id5))
    plt.clf()

    for config5 in sorted(data5[env_id5].keys()):
        xs5, ys5 = zip(*data5[env_id5][config5])
        xs5, ys5 = pad(xs5), pad(ys5)
        assert xs5.shape == ys5.shape
